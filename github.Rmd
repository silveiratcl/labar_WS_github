---
pagetitle: "Git-Github | Github WS"
editor_options: 
  markdown: 
    wrap: 72
---
...
icon_chat <- icons::fontawesome$solid$`comments`
```
...
## 3. Instalando Git-Github-RStudio 
...
### 3.1 Configurando o Git no RStudio
...
### 3.2 Criando repositório (ou REPO) no GitHub 
...
### 3.3 Criando projeto no RStudio e conetando e clonando o repo do Github
...
## 4. Workflow RStudio/Github

### 4.1 Básico: `commit` e `push`
...
### 4.2 Intermediário: `branch` e `merge`
...
### 4.3 Avançado: `fork`, `branch`, `pull request` e `merge`
...
|`branch` `r icon_branch`    | Cria um novo "ramo" em seu repositório                      |
|`pull request` `r icon_pr`  | Solicitação de contribuição                                   |
|`merge` `r icon_merge`      | Incorporar um ramo criado ao ramo "master"                  |
<br><br>
  

Para colaborar em um repositório você antes de mais nada copiar os arquivos para o seu **Github**. Essa ação é executada com o comando `fork` `r icon_fork`. Por exemplo, se dirija ao [respositório de teste](https://github.com/ufsclabar/repo_test) e localize no alto da página o botão `fork` `r icon_fork`. Depois escolha onde você gostaria de colocar, escolha seu usuário. No canto esquerdo da página mostrará que o repo está em seu usuário também. 

<img src= "figs/git_22.png" style= "width:60%">
<br><br>
  

Agora faça novamente os passos descritos em  **3.2 Criando repositório no GitHub (ou REPO)**. Tenha certeza de criar uma nova pasta fora das pastas que você já está trabalhando.

Faça alguma alteração no script, insira seu nome como indicado no arquivo, depois faça `commit` `r icon_commit` e `push` `r icon_push`. Note que seu repositório será atualizado. Navegue até o seu respositório recém clonado e clique em **Contribute**.
...
Agora que você fez um `pull request` `r icon_pr`, basta você aguardar o adminstrador do repositório analisar sua sugestão e fazer o `merge` `r icon_merge` no ramo **master**. Veremos isso na seção a seguir.


### 4.4 Avançado 2: Analisando o `pull request` 

Agora que foi realizado o `pull request` `r icon_pr`, o proprietário do repositório pode analisar as mundanças propostas e fazer o `merge` `r icon_merge`. Navegando ao repositório localize a guia **Pull requests** e clique.                                         
<br><br>
  

<img src= "figs/git_28.png" style= "width:60%">
<br><br>
  
  
Você verá algo como a figura abaixo, indicando os `pull request` `r icon_pr` que podem ser analizados. Clique nos disponíveis, o usuário que fez o `pull request` `r icon_pr` escreveu **Agora a Clarice**, esse foi o comentário realizado no `commit` `r icon_commit`.  

<img src= "figs/git_29.png" style= "width:60%">
<br><br>  

Abrindo o  `pull request` `r icon_pr` você verá algumas guias como: **conversation** `r icon_chat` (conversa sobre as mundanças), **commit** `r icon_commit` (lista de commits), **checks** `r icon_check` (conferidos) e **files changed**.     

<img src= "figs/git_30.png" style= "width:60%">
<br><br>  
...
<img src= "figs/git_31.png" style= "width:60%">
<br><br>  


Depois de analisar as mundanças você pode fazer `merge` `r icon_merge` , ou seja, incorporar a mudança ao ramo **master**. Na guia `pull request` `r icon_pr` aparecerá o ícone `merge` `r icon_merge` lilás, indicando que a mundança foi incorporada ao ramo **master**. 


<img src= "figs/git_32.png" style= "width:60%">
<br><br>  
...
## 5. Back to the future: voltando para uma versão do passado

### 5.1 Visualizando todas as versões do repositório
...
**1)** Você pode visualizar a lista de versões de seu diretório usando o comando `git log --oneline`. Em amarelo são apresentados os códigos dos `commit` `r icon_commit`, em branco as mensagens escritas em cada um deles. A primeira linha representa o `commit` `r icon_commit` mais recente. Se quiser visualizar mais detalhes dos `commits` `r icon_commit` digite somente `git log` 

<img src= "figs/git_33.png" style= "width:60%">
<br><br>  
...
<img src= "figs/git_34.png" style= "width:60%">
<br><br>  
...
<img src= "figs/git_35.png" style= "width:60%">
<br><br>  


### 5.2 Voltando para um determinado commit (ou versão) do repositório **local**

Para voltar a uma determinada versão você deve identificar qual `commit` `r icon_commit`. Anote o **id** e rode o seguinte comando no **terminal**:
...
> **Importante**
> Não esqueça de colocar o ponto "." no final comando. Você não será advertido de nada e vai parecer que funcionou. Sem o "." no final do comando >você vai sair do ramo e vai poder ficar fazendo commits, e estes commits não serão salvos. Portanto, não esqueça o ponto final "."!

<br><br>
  

Após este comando você irá para a versão escolhida **local**.


### 5.3 Adicionando a versão atual para fazer o "commit" e o "push" para o repositório **remoto**

Da mesma maneira que você faz os `commits` `r icon_commit` normais, você tem que também adicionar todos os arquivos desta versão e fazer o `push` `r icon_push`. Abaixo fazendo o commit com linha de comando no **terminal**. 

```{r eval=FALSE }

$ git add .
$ git commit -m "Voltando para o commit id xxxxxx"
$ git push
